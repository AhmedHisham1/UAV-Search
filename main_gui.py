...
from mavsdk_utils import connect_sitl, get_relative_altitude, get_euler_angles, get_lat_lon, land, square_mission, square_mission_offboard, search_mission
import asyncio
...
async def mission(search_lat, search_lon, show_frames=True):
    times = []
    person_lats_all = []
    person_lons_all = []
    ...
    with tf.device('/gpu:0'):
        if detector == 'yolo' or detector == 'yolo-tiny':
            ...
            try:
                drone, home_lat, home_lon = await search_mission(search_lat=search_lat, search_lon=search_lon,
                                                                 mission_alt=5, mission_spd=50, mission_north=10, 
                                                                                                mission_east=-10, 
                                                                                                mission_south=-10, 
                                                                                                RTL_alt=5, 
                                                                                                CAM_pitch=-90, 
            ...
            with detection_graph.as_default():
                with tf.Session(graph=detection_graph, config=config) as sess:
                    ...
                    drone, home_lat, home_lon = await search_mission(search_lat=search_lat, search_lon=search_lon,
                                                                     mission_alt=5, mission_spd=50, mission_north=12, 
                                                                                                    mission_east=-20, 
                                                                                                    mission_south=-3, 
                                                                                                    RTL_alt=5, 
                                                                                                    CAM_pitch=-90, 
...
def button_callback():
    global detector
    detector = detector_feild.get()
    search_lat = float(lat_feild.get())
    search_lon = float(lon_feild.get())
    loop = asyncio.get_event_loop()
    loop.run_until_complete(mission(search_lat, search_lon))


if __name__ == "__main__":
    app = tk.Tk()
    app.title('Drone Manager')
    app.geometry('420x220')
    location_label = tk.Label(app, text='Specify Search Location', font=('bold', 14), pady=10)
    location_label.grid(row=0, columnspan=3, sticky=tk.N)
    lat_label = tk.Label(app, text='Latitude:', font=('bold', 12))
    lat_label.grid(row=1, column=0, sticky=tk.W)
    lon_label = tk.Label(app, text='Longitude:', font=('bold', 12))
    lon_label.grid(row=2, column=0, sticky=tk.W)
    lat_feild = tk.Entry(app)
    lat_feild.grid(row=1, column=1)
    lon_feild = tk.Entry(app)
    lon_feild.grid(row=2, column=1)
    detector_label = tk.Label(app, text='Specify the Type of Object Detection Model', font=('bold', 14), pady=10)
    detector_label.grid(row=3, columnspan=3)
    detector_feild = tk.Entry(app)
    detector_feild.grid(row=4, column=0)
    print_button = tk.Button(app, text='Send Drone', width=20, height=1, command=button_callback)
    print_button.grid(row=7, column=1, sticky=tk.S)
